...
def fuzzit(content):
	...
	n = 50
	for i in range(n):
		byte_array = bytearray(content, 'ascii')
		iterations = random.randint(0, 2*len(byte_array))
		for iteration in range(iterations):
			index_to_modify = random.randint(0, len(byte_array)-1)
			replacement = random.randint(0, 255)
			byte_array[index_to_modify] = replacement
		f.append(str(byte_array))
	return f
